...
class Dealer(set):
    ...
    def get_hand_results(self, hand):
        """ Get cards on table and add them to hand. Get results """
        for x in range(3, 8):
            hand['card%s' % x] = self.get_card()
        
        hand_list = [hand[x] for x in hand]
        return poker_hand(hand_list)
